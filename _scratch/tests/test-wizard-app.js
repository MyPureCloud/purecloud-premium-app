...
import { _renderModule, _renderCompletePage, Page } from './simple-hbs-framework.js'
...
class WizardApp {
    constructor(){
        ...
        this.landingPage = this._createLandingPage();
        this.checkInstallationPage = this._createCheckInstallationStatus();
    }
     ...
     */
    _loadDefaultOrder(){
        let fileUri = this.defaultOrderFileName + ".json";
    ...
    /**
     * Changes the page to a different one  
     * @param {Page} newPage 
     */
    changePage(newPage){
        newPage.load();
    ...
    _createLandingPage(){
        let page = new Page(
            $.proxy(this.getOrganizationInfo, this), 
            null,
            "App Setup Wizard", 
            "Welcome! This Wizard will assist you in the installation, modification, or removal of the Premium App.",
            null,
            hb['landing-page']
        );

        page.assignHandlers = () => {
            $('#btn-check-installation').click(() => {
                this.changePage(this.checkInstallationPage);
            });
        }
    ...
    _createCheckInstallationStatus(){
        let page = new Page(
            $.proxy(this._loadDefaultOrder, this), 
            () => {
                this.getExistingGroups()
                .then(data => {
                    page.renderModule([hb['existing-objects'], data.context, 'results-group', data.group]);
                });
            },
            "Checking Installation", 
            "Check any existing PureCloud Objects that is set up by the App.",
            {
