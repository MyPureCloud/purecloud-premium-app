...
function setupDropdownSelector(currentlySelected) {
  ...
  Object.keys(config.availableLanguageAssets).forEach((langKey) => {
    const newOption = `<option value="${langKey}" ${
      currentlySelected === langKey ? "selected" : ""
    }>${config.availableLanguageAssets[langKey]}</option>`;
...
export async function setPageLanguage(requestedLanguage) {
  ...
  if (
    Object.keys(config.availableLanguageAssets).includes(
      currentLanguageCode
    ) === false
  ) {
...
export function getTranslatedText(key) {
  if (
    localizedLabels[currentLanguageCode] &&
    localizedLabels[currentLanguageCode][key]
  ) {
