...
let modules = [roleModule, OAuthClientModule];
...
async function getIntegrationId() {
  try {
    const clientApps = await integrationsApi.getIntegrationsClientapps({
      pageSize: 1000,
    });
    let instances = clientApps.entities;
    let pa_instance = instances.find(
      (instance) =>
        instance.integrationType.id == config.premiumAppIntegrationTypeId
    );
...
export default {
  ...
  setup(pcClient, user) {
    ...
    modules = modules.filter((module) => {
      return Object.keys(config.provisioningInfo).includes(
        module.provisioningInfoKey
      );
    });
  },
  ...
  async install() {
    ...
    try {
      ...
      modules.forEach((module) => {
        configurationPromises.push(
          module.configure(view.setLoadingModal, installedData, userMe.id)
        );
      });
    ...
    try {
      ...
      let integrationInstance =
        await integrationsApi.getIntegrationConfigCurrent(integrationId);
  ...
  uninstall() {
    ...
    modules.forEach((module) => {
      promiseArr.push(module.remove(view.showLoadingModal));
    });
